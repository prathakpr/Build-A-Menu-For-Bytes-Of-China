...
INSERT INTO category VALUES ('LS', 'Luncheon Specials', 'Served with Hot and Sour Soup or Egg Drop Soup and Fried or Steamed Rice between 11:00 am and 3:00 pm from Monday to Friday.');
INSERT INTO category VALUES ('HS', 'House Specials', null);
...
SAMPLLE QUERIES
*/
SELECT restaurant.name AS restaurant_name, address.street_number AS street_number, address.street_name AS street_name
FROM restaurant
JOIN address
...
SELECT category.name AS category, dish.name AS dish_name,
...
SELECT dish.is_hot_spicy AS spicy_dish_name, category.name AS category, categories_dishes.price AS price
FROM dish
JOIN categories_dishes
ON dish.id = categories_dishes.dish_id
JOIN category
ON category.id = categories_dishes.category_id
WHERE dish.is_hot_spicy = true;
